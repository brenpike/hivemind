...
This section's soundness argument covers the transport only. It rests on two properties that hold by construction: (1) the Write `file_path` is a skill-body literal — a fixed prefix plus a skill-body-authored `<token>` — so no caller-supplied value steers where the Write lands; (2) the file's content reaches its engine only through `jq`, bound into shell variables referenced as `"$var"`, so no field is evaluated as Bash, executed, or read as an instruction. What an engine does with a field after reading it — store it verbatim, validate it as an identifier, or resolve it as a filesystem path — is that engine's own contract, declared in its skill's Inputs JSON schema and enforced by its script. Where an engine resolves a path, the **Trust-Boundary Discipline** section above governs that resolution. See that section and ADR-0019.
