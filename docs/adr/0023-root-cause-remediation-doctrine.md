...
The NORMATIVE home for this routing — the conjunctive conditions, the destination-by-role rule, the code-comment floor, and the promotion path — is `plugin/governance/remediation-doctrine.md` (`## Defer-with-Scope`, `### Recorded Residual`). This amendment records the decision and its rationale; the governance doc executes it. **The decision summary in this amendment is a NON-AUTHORITATIVE ECHO**, written so the rationale reads as a whole — never a second source of the mechanics. Where this echo diverges from, narrows, or widens the governance text, THE GOVERNANCE DOC GOVERNS and the echo is stale prose to be corrected. In particular, the three recorded-residual conditions are CONJUNCTIVE — all three must hold — and an agent applies them from governance, never from this summary. The pointer stays one-way ADR→governance.
...
This amendment is APPEND-ONLY in the EDITING sense: the original Context, Decision, Considered Options, and Consequences above are left byte-unchanged rather than rewritten in place. It is NOT append-only in NORMATIVE FORCE. Where this amendment supersedes a clause above — Decision 4 and the Consequences naming "a tracked issue" as the structural home for a finding left unfixed — THIS AMENDMENT GOVERNS and the superseded clause is historical record, not an enforceable rule. Every clause this amendment does not name stands in force as written. Status remains accepted.
