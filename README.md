...
| `hivemind:decision-report` | The AUTOMATIC post-merge invocation is opt-in and off by default — enable it by setting `HIVEMIND_ENABLE_DECISION_REPORT` in the `env` block of `.claude/settings.json` or `.claude/settings.local.json`; the key gates only that automatic path, and invoking the skill explicitly still works regardless of the key. When enabled, after a run's PR merges or closes, render a chronological narrative of the auto-decisions the run made on the user's behalf — in the consumer project's domain language — and return it as chat text (render-to-chat; writes nothing to disk — the overlord surfaces the narrative and touches a zero-byte done-marker) |
