...
- `governance/safety-rails.md`: the Destructive Fix Gate list now matches `governance/security-policy.md` word for word. This broadens what the gate stops: key sizes, salt rounds, and TLS settings; `Cargo.toml` and `go.mod` dependency additions; `*.p12`, `credentials.*`, and `secrets.*` files; and `.circleci/` workflow files.
